...
public class LocalHub implements WebQueryInterface, WebToolReportingInterface,
	...
	public static LocalHubDebugAccess startTESTINGServerAndReturnDebugAccess(
			String screencastMonitorLocation) {
		return startServerAndReturnDebugAccess(screencastMonitorLocation,
				TestingUtils.TEST_DB_LOC, false, false, false);
	}

	private static LocalHubDebugAccess startServerAndReturnDebugAccess(
			String screencastMonitorLocation, String databaseLocation,
			boolean wantHTTP, boolean wantScreenRecording,
			boolean wantRemoteToolReporting) {
		return startServer(screencastMonitorLocation, databaseLocation,
				wantHTTP, wantScreenRecording, wantRemoteToolReporting, true);
	}

	public static LocalHubDebugAccess startServer(
			String screencastMonitorLocation, String databaseLocation,
			boolean wantHTTP, boolean wantScreenRecording,
		...
		if (!singletonHub.isRunning()) {
			...
			singletonHub
					.setScreencastMonitorLocation(screencastMonitorLocation);
			singletonHub.start();
		}
	...
	public static LocalHubProcess startServerForUse(
			String screencastMonitorLocation) {
		return startServerForUse(screencastMonitorLocation,
				LocalSQLDatabaseFactory.DEFAULT_SQLITE_LOCATION);
	}

	public static LocalHubProcess startServerForUse(
			String screencastMonitorLocation, String databaseLocation) {
		return startServer(screencastMonitorLocation, databaseLocation, true,
				true, true, false);
	}
	...
	private void start() {
		if (isRunning() || this.screencastMonitorDirectory == null) {
			logger.info("Did not start the server because "
					+ (isRunning() ? "it was already running"
							: " no monitor directory had been set."));
			return;
		}
		isRunning = true;
		...
		this.screenRecordingModule = new ScreenRecordingModule(
				this.screencastMonitorDirectory);
		...
		this.postProductionHandler = new PostProductionHandler(
				this.screencastMonitorDirectory, userManager);
		...
		if (shouldReportToolsRemotely) {
			...
			this.remoteToolReporter = new RemoteToolReporter(
					this.databaseManager, userManager, this.notificationManager);
		}
	}
	...
	private void setUpUserManager() {
		...
		// }
		userManager = new UserManager(initDirectory); // this will block and
	...
	private void setScreencastMonitorLocation(String screencastMonitorLocation) {
		if (screencastMonitorLocation == null
				|| screencastMonitorLocation.isEmpty()) {
			if (screencastMonitorDirectory == null) {
				logger.fatal("Invalid input into setMonitorLocation");
				throw new RuntimeException(
						"Could not set the monitorLocation to "
								+ screencastMonitorLocation);
			}
		...
		if (!newMonitorDirectory.exists()) {
			if (!newMonitorDirectory.mkdirs()) {
				if (screencastMonitorDirectory == null) {
					...
					throw new RuntimeException(
							"Could not create the monitor directory "
									+ newMonitorDirectory);
				}
				logger.error("Could not create the monitor directory "
						+ screencastMonitorLocation
						+ ", continuing with old directory");
				return;
			}
			...
			logger.error("Could not set the monitor directory to be "
					+ screencastMonitorLocation
					+ ", because it is not a directory.  Continuing with old directory.");
			return;
		} else {
	...
	private void setDatabaseManager(String databaseLocation) {
		this.databaseManager = BufferedDatabaseManager
				.createBufferedDatabasemanager(databaseLocation,
						this.userManager, this.isDebug);
	}
	...
	@Override
	public ToolCountStruct getToolAggregate(String applicationName,
			String toolName) {
		return databaseManager.getToolAggregate(applicationName, toolName);
	}

	@Override
	public void reportToolStream(ToolStream ts) // requests coming in from the
												// web (usually async)
	{
		logger.debug("waiting in line at localHub");
		synchronized (this) {
			logger.info("ToolStream Reported from Plugin: "
					+ ts.getAssociatedPlugin());
	...
	private void cleanUpObsoleteClips() {
		// Query database for clips made and update the database with those that
		// don't exist
		...
		for (String path : pathsToDelete) {
			...
			if (clipToDelete.exists()) {
				if (TestingUtils.clearOutDirectory(clipToDelete)
						&& clipToDelete.delete()) {
	...
	private void potentiallyMakeClipsFromToolStream(ToolStream ts) {
		if (pluginIsHidden(ts.getAssociatedPlugin())) {
			logger.debug("Not making screencasts for "
					+ ts.getAssociatedPlugin() + " because it is hidden");
			return;
		}
		...
		for (ToolUsage tu : ts.getAsList()) {
			if (clipQualityManager.shouldMakeClipForUsage(tu)) {
				if (shouldUseScreenRecording || isDebug) {
					...
					try {
						this.postProductionHandler
								.extractBrowserMediaForToolUsage(tu);
						String clipPath = FileUtilities
								.makeLocalFolderNameForBrowserMediaPackage(tu,
										getCurrentUserEmail());
						this.databaseManager.reportMediaMadeForToolUsage(
								clipPath, tu);
					} catch (MediaEncodingException e) {
						logger.error("Problem making media for " + tu, e);
					}
				} else {
					logger.debug("Not making clip from " + tu
							+ " because screencasting is turned off");
				}
			} else {
				logger.debug("Not making clip from " + tu
						+ " because its score isn't high enough");
			}
		}
	}
	...
	public List<File> getBestExamplesOfTool(String pluginName, String toolName,
			boolean isKeyboardHuh) {
		List<ToolUsage> usages = this.databaseManager
				.getBestNInstancesOfToolUsage(5, pluginName, toolName,
						isKeyboardHuh);
		...
		for (ToolUsage toolUsage : usages) {
			File potentialFile = new File(
					FileUtilities.makeLocalFolderNameForBrowserMediaPackage(
							toolUsage, userManager.getUserEmail()));
			if (potentialFile.exists() && potentialFile.isDirectory()) {
				retVal.add(potentialFile);
			} else {
				logger.error("Clip does not exist for " + toolUsage
						+ ", but it should...");
			}
		}
	 ...
	 * 
	 * @see
	 * edu.ncsu.lubick.localHub.WebQueryInterface#shareClipWithUser(java.lang
	 * .String, java.lang.String, edu.ncsu.lubick.localHub.ClipOptions)
	 */
	@Override
	public void shareClipWithUser(String clipId, String recipient,
		...
		updateClipOptions(PostProductionHandler.MEDIA_OUTPUT_FOLDER + clipId,
				clipOptions, false);
	...
	@Override
	public void requestClipsFromUser(String owner, String pluginName,
			String toolName) {
		this.clipShareRequester.requestClipsFromUser(owner, pluginName,
				toolName);
	}
	...
	public void notificationReceived(String notifications) {
		...
		ToolUsage tu = new ToolUsage("Notification received", notifications,
				"[GUI]", "[ScreencastingHub]", new Date(), 10, 10);
		reportToolUsage(tu);
	}
	...
	public void notificationClickedOn(String notification) {
		// Log notification for study
		ToolUsage tu = new ToolUsage("Respond to notification", notification,
				"[GUI]", "[ScreencastingHub]", new Date(), 10, 10);
		reportToolUsage(tu);
	}

	@Override
	public void updateClipOptions(String clipId, ClipOptions options,
			boolean upload) {
		// TODO fix or remove
		// databaseManager.setStartEndFrame(folder, options.startFrame,
		// options.endFrame);
	...
	public void updateActivity(String pluginName, boolean isActive) {
		if (isActive) {
			...
			if (!areAnyActive && pausingTimerTask == null) { // don't schedule
																// the task
																// twice
