...
public class FrameDecompressor implements FrameDecompressorCodecStrategy, FrameDecompressorReadingStrategy
{
	...
	public int readFrameType(InputStream inputStream) throws IOException
	{
		...
		// Convert the type to a positive int. It shouldn't be otherwise, but
		// just in case
		return (type & 0xFF);
	}
	...
	private void decodeFramePacketUsingRunTimeEncoding(DecompressionFramePacket packet)
	{
		...
		logger.debug(String.format("Ending inCursor: %d/%d and outCursor: %d/%d", inCursor, packet.encodedData.length - 1, outCursor,
				packet.decodedData.length - 1));

		// Many times, if we are on the last couple of bytes, we don't read the
		// last 2 bytes that look something like
		// [..., -1, 62] and these are ignored because the loop stops if we are
		// past the 3rd to last byte
		// So, the best way to fix this is just assume if we haven't filled the
		// expected size, copy all the stuff from the end
	...
	private int readCompressedFrameSize(InputStream inputStream) throws IOException, ReachedEndOfCapFileException
	{
		...
		{
			zSize = zSize << 8; // done here to effectively have the bit
								// shifting only done 3 times
